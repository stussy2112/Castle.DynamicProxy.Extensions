// -----------------------------------------------------------------------
// <copyright file="ServiceCollectionExtensions.TryAddTransient.cs" company="Karma, LLC">
...
namespace Microsoft.Extensions.DependencyInjection
{
  ...
  {
    /// <summary>
    /// Attempts to add a transient service of the specified type to the dependency injection container with the
    /// provided interceptors if it has not already been registered.
    /// </summary>
    /// <remarks>
    /// Transient services are created each time they are requested. If the service type has already been registered,
    ...
    /// <returns>The <see cref="IServiceCollection"/> instance with the intercepted transient service registration added if it was
    /// not already present.</returns>
    public static IServiceCollection TryAddInterceptedTransient(
      this IServiceCollection services,
      [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors)] Type serviceType,
      params IInterceptor[] interceptors) =>
      services.TryAddIntercepted(serviceType, ServiceLifetime.Transient, interceptors);
    ...
    /// the order specified. If the service type is already registered, no changes are made.
    /// </remarks>
    /// <param name="services">The <see cref="IServiceCollection"/> to which the service will be added.</param>
    /// <param name="serviceType">The type of the service to register. Must have a public constructor.</param>
    ...
    public static IServiceCollection TryAddInterceptedTransient(
      ...
      services.TryAddIntercepted(serviceType, ServiceLifetime.Transient, interceptorTypes);

    /// <summary>
    /// Attempts to add a transient service of the specified type and implementation to the dependency injection
    ...
    public static IServiceCollection TryAddInterceptedTransient(
      ...
      params IInterceptor[] interceptors) =>
      services.TryAddIntercepted(serviceType, implementationType, ServiceLifetime.Transient, interceptors);
    ...
    public static IServiceCollection TryAddInterceptedTransient(
      ...
      [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors)] Type implementationType,
      [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors)] params Type[] interceptorTypes) =>
      services.TryAddIntercepted(serviceType, implementationType, ServiceLifetime.Transient, interceptorTypes);

    /// <summary>
    /// Attempts to add a transient service of the specified type to the dependency injection container using a factory
    ...
    /// requested, and the resulting instance is wrapped with the specified interceptors. If the service type is
    ...
    public static IServiceCollection TryAddInterceptedTransient(
      ...
      params IInterceptor[] interceptors) =>
      services.TryAddIntercepted(serviceType, implementationFactory, ServiceLifetime.Transient, interceptors);
    ...
    /// each time the service is requested. If the service type is already registered, no changes are made.
    /// </remarks>
    /// <param name="services">The <see cref="IServiceCollection"/> to which the service will be added.</param>
    /// <param name="serviceType">The type of the service to register.</param>
    /// <param name="implementationFactory">A factory function that creates an instance of the service. The function receives the
    ...
    public static IServiceCollection TryAddInterceptedTransient(
      ...
      Func<IServiceProvider, object> implementationFactory,
      [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors)] params Type[] interceptorTypes) =>
      services.TryAddIntercepted(serviceType, implementationFactory, ServiceLifetime.Transient, interceptorTypes);

    /// <summary>
    /// Attempts to add a transient service of type <typeparamref name="TService"/> to the dependency injection
    ...
    /// <returns>The <see cref="IServiceCollection"/> instance with the intercepted transient service registration added if it was
    /// not already present.</returns>
    public static IServiceCollection TryAddInterceptedTransient<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors)] TService>(
      this IServiceCollection services,
      Func<IServiceProvider, TService> implementationFactory,
      params IInterceptor[] interceptors)
      where TService : class =>
      services.TryAddIntercepted(implementationFactory, ServiceLifetime.Transient, interceptors);
    ...
    public static IServiceCollection TryAddInterceptedTransient<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors)] TService>(
      ...
      [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors)] params Type[] interceptorTypes)
      where TService : class =>
      services.TryAddIntercepted(implementationFactory, ServiceLifetime.Transient, interceptorTypes);
    ...
    /// injection container. If the service type is already registered, no changes are made.
    /// </remarks>
    /// <typeparam name="TService">The type of the service to register. Must be a reference type with a public constructor.</typeparam>
    ...
    /// <returns>The <see cref="IServiceCollection"/> instance with the intercepted transient service registration added if it was
    /// not already present.</returns>
    public static IServiceCollection TryAddInterceptedTransient<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors)] TService, [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors)] TInterceptor>(
      this IServiceCollection services,
      Func<IServiceProvider, TService> implementationFactory)
      where TService : class
      where TInterceptor : IInterceptor =>
      services.TryAddIntercepted<TService, TInterceptor>(implementationFactory, ServiceLifetime.Transient);
    ...
    /// service type is already registered, no changes are made.
    /// </remarks>
    /// <typeparam name="TService">The type of the service to register. Must be a reference type with a public constructor.</typeparam>
    /// <param name="services">The <see cref="IServiceCollection"/> to which the service will be added.</param>
    /// <param name="interceptors">An array of <see cref="IInterceptor"/> instances to be applied to the service implementation.
    ...
    public static IServiceCollection TryAddInterceptedTransient<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors)] TService>(
      ...
      params IInterceptor[] interceptors)
      where TService : class =>
      services.TryAddIntercepted<TService>(ServiceLifetime.Transient, interceptors);
    ...
    public static IServiceCollection TryAddInterceptedTransient<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors)] TService>(
      ...
      [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors)] params Type[] interceptorTypes)
      where TService : class =>
      services.TryAddIntercepted<TService>(ServiceLifetime.Transient, interceptorTypes);
    ...
    /// concerns such as logging, validation, or authorization. If the service type is already registered, no changes
    /// are made.
    /// </remarks>
    /// <typeparam name="TService">The type of the service to register. Must be a reference type with a public constructor.</typeparam>
    /// <typeparam name="TInterceptor">The type of the interceptor to apply. Must implement <see cref="IInterceptor"/> and have a public
    ...
    /// <returns>The <see cref="IServiceCollection"/> instance with the intercepted transient service registration added if it was
    /// not already present.</returns>
    public static IServiceCollection TryAddInterceptedTransient<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors)] TService, [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors)] TInterceptor>(this IServiceCollection services)
      where TService : class
      where TInterceptor : IInterceptor =>
      services.TryAddIntercepted<TService, TInterceptor>(ServiceLifetime.Transient);

    /// <summary>
    /// Attempts to add a transient service of type <typeparamref name="TService"/> with implementation type
    ...
    /// <returns>The <see cref="IServiceCollection"/> instance with the intercepted transient service registration added if it was
    /// not already present.</returns>
    public static IServiceCollection TryAddInterceptedTransient<TService, [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors)] TImplementation>(
      this IServiceCollection services,
      params IInterceptor[] interceptors)
      where TService : class
      where TImplementation : class, TService =>
      services.TryAddIntercepted<TService, TImplementation>(ServiceLifetime.Transient, interceptors);
    ...
    public static IServiceCollection TryAddInterceptedTransient<TService, [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors)] TImplementation>(
      ...
      where TService : class
      where TImplementation : class, TService =>
      services.TryAddIntercepted<TService, TImplementation>(ServiceLifetime.Transient, interceptorTypes);
    ...
    /// <returns>The <see cref="IServiceCollection"/> instance with the intercepted transient service registration added if it was
    /// not already present.</returns>
    public static IServiceCollection TryAddInterceptedTransient<TService, [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors)] TImplementation, [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors)] TInterceptor>(this IServiceCollection services)
      where TService : class
      where TImplementation : class, TService
      where TInterceptor : IInterceptor =>
      services.TryAddIntercepted<TService, TImplementation, TInterceptor>(ServiceLifetime.Transient);
  }
}

// -----------------------------------------------------------------------
// <copyright file="ServiceCollectionExtensions.AddKeyedScoped.cs" company="Karma, LLC">
...
namespace Microsoft.Extensions.DependencyInjection
{
  ...
  {
    /// <summary>
    /// Adds a keyed scoped service of the specified type to the dependency injection container with the provided
    /// interceptors.
    /// </summary>
    /// <remarks>
    /// Scoped services are created once per client request (scope). This method registers the service with a specific
    /// key so that all resolved instances within the same scope will be wrapped with the specified interceptors. Use
    /// this to enable cross-cutting concerns such as logging, validation, or authorization for keyed scoped services.
    /// </remarks>
    ...
    /// <returns>The <see cref="IServiceCollection"/> instance with the intercepted keyed scoped service registration added.</returns>
    public static IServiceCollection AddKeyedInterceptedScoped(
      this IServiceCollection services,
      [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors)] Type serviceType,
      object? serviceKey,
      params IInterceptor[] interceptors) =>
      services.AddKeyedIntercepted(serviceType, serviceKey, ServiceLifetime.Scoped, interceptors);
    ...
    /// order specified. Use this for cross-cutting concerns such as logging, validation, or authorization.
    /// </remarks>
    /// <param name="services">The <see cref="IServiceCollection"/> to which the service will be added.</param>
    /// <param name="serviceType">The type of the service to register. Must have a public constructor.</param>
    /// <param name="serviceKey">The key that uniquely identifies the keyed service registration. Pass null to use null as the key.
    ...
    public static IServiceCollection AddKeyedInterceptedScoped(
      ...
      services.AddKeyedIntercepted(serviceType, serviceKey, ServiceLifetime.Scoped, interceptorTypes);

    /// <summary>
    /// Adds a keyed scoped service of the specified type and implementation to the dependency injection container with
    ...
    public static IServiceCollection AddKeyedInterceptedScoped(
      ...
      params IInterceptor[] interceptors) =>
      services.AddKeyedIntercepted(serviceType, serviceKey, implementationType, ServiceLifetime.Scoped, interceptors);
    ...
    public static IServiceCollection AddKeyedInterceptedScoped(
      ...
      [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors)] Type implementationType,
      [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors)] params Type[] interceptorTypes) =>
      services.AddKeyedIntercepted(serviceType, serviceKey, implementationType, ServiceLifetime.Scoped, interceptorTypes);

    /// <summary>
    /// Adds a keyed scoped service of the specified type to the dependency injection container using a factory method
    ...
    /// resulting instance is wrapped with the specified interceptors.
    /// </remarks>
    /// <param name="services">The <see cref="IServiceCollection"/> to which the service will be added.</param>
    ...
    public static IServiceCollection AddKeyedInterceptedScoped(
      ...
      params IInterceptor[] interceptors) =>
      services.AddKeyedIntercepted(serviceType, serviceKey, implementationFactory, ServiceLifetime.Scoped, interceptors);
    ...
    public static IServiceCollection AddKeyedInterceptedScoped(
      ...
      Func<IServiceProvider, object?, object> implementationFactory,
      [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors)] params Type[] interceptorTypes) =>
      services.AddKeyedIntercepted(serviceType, serviceKey, implementationFactory, ServiceLifetime.Scoped, interceptorTypes);

    /// <summary>
    /// Adds a keyed scoped service of type <typeparamref name="TService"/> to the dependency injection container using
    /// a factory method and the provided interceptors.
    /// </summary>
    /// <remarks>
    /// Scoped services are created once per client request (scope). This method uses a factory function to create the
    /// service instance, allowing for custom initialization logic. The factory is invoked once per scope, and the
    ...
    /// Interceptors are invoked in the order provided.</param>
    /// <returns>The <see cref="IServiceCollection"/> instance with the intercepted keyed scoped service registration added.</returns>
    public static IServiceCollection AddKeyedInterceptedScoped<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors)] TService>(
      this IServiceCollection services,
      object? serviceKey,
      Func<IServiceProvider, object?, TService> implementationFactory,
      params IInterceptor[] interceptors)
      where TService : class =>
      services.AddKeyedIntercepted(serviceKey, implementationFactory, ServiceLifetime.Scoped, interceptors);

    /// <summary>
    /// Adds a keyed scoped service of type <typeparamref name="TService"/> to the dependency injection container using
    /// a factory method and the provided interceptor types.
    /// </summary>
    /// <remarks>
    /// Scoped services are created once per client request (scope). This method uses a factory function to create the
    /// service instance, with interceptors resolved from the dependency injection container. The factory is invoked once
    ...
    public static IServiceCollection AddKeyedInterceptedScoped<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors)] TService>(
      ...
      [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors)] params Type[] interceptorTypes)
      where TService : class =>
      services.AddKeyedIntercepted(serviceKey, implementationFactory, ServiceLifetime.Scoped, interceptorTypes);

    /// <summary>
    /// Adds a keyed scoped service of type <typeparamref name="TService"/> to the dependency injection container using
    /// a factory method and the specified interceptor type.
    /// </summary>
    /// <remarks>
    /// Scoped services are created once per client request (scope). This method uses a factory function to create the
    ...
    /// current <see cref="IServiceProvider"/> and the service key.</param>
    /// <returns>The <see cref="IServiceCollection"/> instance with the intercepted keyed scoped service registration added.</returns>
    public static IServiceCollection AddKeyedInterceptedScoped<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors)] TService, [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors)] TInterceptor>(
      this IServiceCollection services,
      object? serviceKey,
      Func<IServiceProvider, object?, TService> implementationFactory)
      where TService : class
      where TInterceptor : IInterceptor =>
      services.AddKeyedIntercepted<TService, TInterceptor>(serviceKey, implementationFactory, ServiceLifetime.Scoped);

    /// <summary>
    /// Adds a keyed scoped service of type <typeparamref name="TService"/> to the dependency injection container with
    /// the provided interceptors.
    /// </summary>
    /// <remarks>
    ...
    public static IServiceCollection AddKeyedInterceptedScoped<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors)] TService>(
      ...
      params IInterceptor[] interceptors)
      where TService : class =>
      services.AddKeyedIntercepted<TService>(serviceKey, ServiceLifetime.Scoped, interceptors);
    ...
    public static IServiceCollection AddKeyedInterceptedScoped<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors)] TService>(
      ...
      [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors)] params Type[] interceptorTypes)
      where TService : class =>
      services.AddKeyedIntercepted<TService>(serviceKey, ServiceLifetime.Scoped, interceptorTypes);

    /// <summary>
    /// Adds a keyed scoped service of type <typeparamref name="TService"/> to the dependency injection container with
    /// the specified interceptor type.
    /// </summary>
    /// <remarks>
    ...
    public static IServiceCollection AddKeyedInterceptedScoped<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors)] TService, [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors)] TInterceptor>(
      ...
      where TService : class
      where TInterceptor : IInterceptor =>
      services.AddKeyedIntercepted<TService, TInterceptor>(serviceKey, ServiceLifetime.Scoped);

    /// <summary>
    /// Adds a keyed scoped service of type <typeparamref name="TService"/> with implementation type
    ...
    /// Interceptors are invoked in the order provided.</param>
    /// <returns>The <see cref="IServiceCollection"/> instance with the intercepted keyed scoped service registration added.</returns>
    public static IServiceCollection AddKeyedInterceptedScoped<TService, [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors)] TImplementation>(
      this IServiceCollection services,
      object? serviceKey,
      params IInterceptor[] interceptors)
      where TService : class
      where TImplementation : class, TService =>
      services.AddKeyedIntercepted<TService, TImplementation>(serviceKey, ServiceLifetime.Scoped, interceptors);
    ...
    public static IServiceCollection AddKeyedInterceptedScoped<TService, [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors)] TImplementation>(
      ...
      where TService : class
      where TImplementation : class, TService =>
      services.AddKeyedIntercepted<TService, TImplementation>(serviceKey, ServiceLifetime.Scoped, interceptorTypes);
    ...
    /// Note: Keyed services with null keys are still resolved as keyed services, not as non-keyed services.</param>
    /// <returns>The <see cref="IServiceCollection"/> instance with the intercepted keyed scoped service registration added.</returns>
    public static IServiceCollection AddKeyedInterceptedScoped<TService, [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors)] TImplementation, [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors)] TInterceptor>(
      this IServiceCollection services,
      object? serviceKey)
      where TService : class
      where TImplementation : class, TService
      where TInterceptor : IInterceptor =>
      services.AddKeyedIntercepted<TService, TImplementation, TInterceptor>(serviceKey, ServiceLifetime.Scoped);
  }
}
